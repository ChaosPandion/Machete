...
module internal CharSets =
    ...
    let nonWordCharSet = 
        set [
            for c in [Char.MinValue..Char.MaxValue] do 
                if not (wordCharSet.Contains c) then 
                    yield c
        ]

    let whiteSpaceLineTerminatorCharSet = 
        Set.union whiteSpaceCharSet lineTerminatorCharSet

    let nonWhiteSpaceLineTerminatorCharSet = 
        set [
            for c in [Char.MinValue..Char.MaxValue] do 
                if not (whiteSpaceLineTerminatorCharSet.Contains c) then 
                    yield c
        ]
    ...
    let controlLetterCharSet = set "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"